  ...
  */
  ...
  public:
    ...
    */
    ...
    /// \name Basic usage
    //@{
    /** \brief Minimize functions \c func over \c nv variables beginning
        with starting point \c x0 and returning minimum \c fmin
     */
...
#pragma omp parallel
      ...
      E_store.resize(35*n_threads*this->ntrial);
      x_store.resize(35*n_threads*this->ntrial);
...
#pragma omp for
          ...
          for(size_t it=0;it<n_threads;it++) {
            ...
            for (int i=0;i<this->ntrial;++i) {
              ...
              //ubvector E_store(35*n_threads*this->ntrial);
              size_t ix=iT*n_threads*this->ntrial+it*this->ntrial+i;
              E_store[ix]=new_E[it];
              x_store[ix]=new_x[it][0];
              //std::cout << iT << " " << it << " " << i << " "
              //<< ix << std::endl;
...
#pragma omp for
    ...
    /** \brief Minimize thread-safe function \c func over \c nv
        variables beginning with starting point \c x0 and returning
        minimum \c fmin
     */
    virtual int mmin(size_t nv, vec_t &x0, double &fmin, 
                     func_t &func) {
